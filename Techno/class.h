   ...
   public:
    ...
    void Draw(Param *p) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            p->Draw(x + i * letters[i]->width - distance * i, y,
                    letters[n]->width, letters[n]->height, letters[n]);
        }
    }
    ...
    void Draw(Param *p, int *pos, int s) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            p->Draw(x + i * temp->width - distance * i, y, temp->width,
                    temp->height, temp);
        }
    }
   ...
   public:
    ...
    void MoveR() {
        ...
        if (R == true) {
            ...
            if (MatMap[ny][nx] == 0 || MatMap[ny][nx] == 5 ||
                MatMap[ny][nx] == 3 || MatMap[ny][nx] == 9) {
...
class Inventar {
   ...
   public:
    ...
    void Draw(Param *p) {
        ...
        if (exp == true)
            p->Draw(mX, mY, InventarExp->width, InventarExp->height,
                    InventarExp);
    }
    void ChangeImages() {
...
class Chest {
   ...
   public:
        ...
        if (show == true) {
            ...
            if (LockerLight->Touch(mX, mY) == true && dist > 55 && dist < 88) {
                ...
                if (locker->Touch(X, Y) == true && lmb == true) {
                    ...
                    if (cd[0] < 10)
                        ran = sprintf(showText, "0%d 0%d 0%d", cd[0], cd[1],
                                      cd[2]);
                    else
    ...
    void DrawC(Param *p) {
        ...
        if (showC == true) {
            p->Draw((w - code->width) / 2, (h - code->height) / 2,
            ...
            for (int i = 0; i < 6 * 8; i++) {
                if (objects[i] != 0)
                    p->Draw((w - code->width) / 2 + 71 + (ix)*5 + (ix)*25,
                            ...
                            ImObjects[i]->width, ImObjects[i]->height,
                            ImObjects[i]);
        ...
        for (int i = 0; i < h; ++i)
            for (int j = 0; j < w; ++j)
                memcpy(reinterpret_cast<char *>(lockedRect.pBits) + x * 4 +
                           j * 4 + i * lockedRect.Pitch + y * lockedRect.Pitch,
